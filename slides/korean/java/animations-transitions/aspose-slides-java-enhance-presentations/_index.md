...
# Aspose.Slides for Java를 사용하여 프레젠테이션에서 프레임을 그리고 표에 텍스트 추가하는 방법

## Introduction

PowerPoint에서 데이터를 명확하게 표시하는 것은 특히 **표에 텍스트 추가**가 필요하고 시각적 요소로 중요한 값을 강조해야 할 때 큰 장벽이 될 수 있습니다. 이 가이드에서는 특정 단락 주위에 **프레임을 그리는 방법**, 도형 내부의 텍스트 정렬 설정, 그리고 마지막으로 **프레젠테이션을 pptx로 저장**하는 방법을 Aspose.Slides for Java를 사용해 배우게 됩니다. 끝까지 진행하면 청중의 시선을 정확히 원하는 곳으로 끌어올 수 있는 깔끔한 슬라이드 덱을 만들 수 있습니다.

슬라이드를 돋보이게 할 준비가 되셨나요? 단계별로 과정을 살펴보겠습니다.

## Quick Answers
- **“표에 텍스트 추가”가 무엇을 의미하나요?** 프로그래밍 방식으로 개별 표 셀의 텍스트 내용을 삽입하거나 업데이트하는 것을 의미합니다.  
- **파일을 저장하는 메서드는 무엇인가요?** `pres.save("output.pptx", SaveFormat.Pptx)` – 이 **프레젠테이션을 pptx로 저장** 단계가 변경 사항을 최종 확정합니다.  
- **도형 내부의 텍스트를 어떻게 정렬하나요?** `autoShape.getTextFrame().getParagraphs().get_Item(0).getParagraphFormat().setAlignment(...)`를 통해 `TextAlignment.Left`(또는 Center/Right)를 사용합니다.  
- **단락 주위에 사각형을 그릴 수 있나요?** 예 — 단락을 순회하면서 경계 사각형을 얻고, 채우기 없이 검은 선으로 `IAutoShape`를 추가하면 됩니다.  
- **라이선스가 필요합니까?** 평가용으로는 임시 라이선스로 충분하지만, 실제 운영에서는 정식 라이선스가 필요합니다.  

## Why draw frames around text?

단락이나 특정 부분(예: 문자 **'0'**이 포함된 텍스트) 주위에 프레임(또는 사각형)을 그리면 즉시 주목을 끌 수 있습니다. 이 기법은 다음에 이상적입니다:

- 표에서 핵심 재무 수치를 강조하기.  
- 슬라이드에서 경고나 중요한 메모를 강조하기.  
- 추가 도형을 수동으로 만들지 않고 시각적 구분자를 생성하기.
...
### Environment Setup
Java Development Kit(JDK)이 설치되어 있는지 확인하십시오. 가능하면 JDK 16 이상을 권장합니다. 이 예제는 `jdk16` 분류자를 사용합니다.
...
#### Overview
이 기능은 **표를 생성**하고, **표에 텍스트 추가**한 뒤, **프레젠테이션을 pptx로 저장**하는 과정을 보여줍니다.
...
**2. 셀에 텍스트 추가**  
텍스트 부분을 포함한 단락을 생성하고 특정 셀에 추가합니다.
```java
...
#### Overview
이 예제는 **AutoShape에 TextFrame을 추가하고 정렬을 설정**하는 방법을 보여줍니다—즉, **set text alignment java**의 예시입니다.
...
#### Overview
이 기능은 **텍스트 주위에 프레임을 그리기**와 문자 ‘0’이 포함된 부분에 **단락 주위에 사각형 그리기**를 다룹니다.
...
- **Null 검사** – `Presentation` 사용을 항상 try‑finally 블록으로 감싸 `pres.dispose()`가 실행되어 네이티브 리소스를 해제하도록 합니다.  
- **경계 사각형 정확도** – `para.getRect()`가 반환하는 사각형은 현재 레이아웃을 반영합니다; 글꼴 크기나 여백을 변경하면 프레임을 그리기 전에 사각형을 다시 계산하십시오.  
- **성능** – 매우 큰 표를 다룰 때는 도형 추가를 배치 처리하거나, 업데이트된 기하 정보를 가진 단일 `IAutoShape` 인스턴스를 재사용하여 메모리 오버헤드를 줄이는 것을 고려하십시오.
