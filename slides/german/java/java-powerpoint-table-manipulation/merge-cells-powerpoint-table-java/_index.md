...
## Schritt 2: Präsentationsobjekt instanziieren
 Instanziieren Sie den`Presentation` Klasse zur Darstellung der PPTX-Datei, mit der Sie arbeiten:
```java
...
## Schritt 4: Tabellenabmessungen definieren
 Definieren Sie die Spalten und Zeilen für Ihre Tabelle. Geben Sie die Breite der Spalten und die Höhe der Zeilen als Arrays von`double`:
```java
...
## Schritt 7: Zellen in der Tabelle zusammenführen
 Um Zellen in der Tabelle zu verbinden, verwenden Sie die`mergeCells` Methode. Dieses Beispiel verbindet Zellen von (1, 1) nach (2, 1) und von (1, 2) nach (2, 2):
```java
