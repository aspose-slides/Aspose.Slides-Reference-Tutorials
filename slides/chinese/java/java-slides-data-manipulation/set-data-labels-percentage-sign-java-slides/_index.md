---
"description": "学习如何使用 Aspose.Slides for Java 在 PowerPoint 演示文稿中设置带有百分号的数据标签。通过分步指导和源代码创建引人入胜的图表。"
...
# 在 Java 幻灯片中设置数据标签百分号


## Aspose.Slides for Java 中设置数据标签百分号的介绍

在本指南中，我们将引导您使用 Aspose.Slides for Java 设置带有百分比符号的数据标签。我们将创建一个包含堆叠柱形图的 PowerPoint 演示文稿，并配置数据标签以显示百分比。
...
我们通过创建系列和数据点来向图表添加数据。在此示例中，我们添加了两个系列及其各自的数据点。
...
就是这样！您已成功使用 Aspose.Slides for Java 创建了一个带有堆叠柱形图的 PowerPoint 演示文稿，并配置了数据标签以显示百分比。
...
通过遵循本指南，您将学习如何使用基于百分比的数据标签创建引人入胜的演示文稿，这对于在商业报告、教育材料等中有效传达信息特别有用。
...
是的，您可以通过设置 `setFontHeight` 属性如代码所示。
