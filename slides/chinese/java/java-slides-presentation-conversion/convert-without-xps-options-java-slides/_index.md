---
"description": "学习如何使用 Aspose.Slides for Java 将 PowerPoint 演示文稿转换为 XPS 格式。包含源代码的分步指南。"
...
在本教程中，我们将指导您使用 Aspose.Slides for Java 将 PowerPoint 演示文稿转换为 XPS（XML 纸张规范）文档，无需指定任何 XPS 选项。我们将提供分步说明和 Java 源代码，帮助您完成此任务。
...
此代码块将演示文稿保存为名为 `"XPS_Output_Without_XPSOption_out.xps"`。您可以根据需要更改输出文件名。

## Java Slides 中不使用 XPS 选项进行转换的完整源代码
...
要在转换 PowerPoint 演示文稿时指定 XPS 选项，您可以使用 `XpsOptions` 类并设置各种属性，例如图像压缩和字体嵌入。如果您对 XPS 转换有特定要求，请参阅 [Aspose.Slides for Java 文档](https://docs.aspose.com/slides/java/) 了解更多详情。

### 是否有其他格式的保存选项？

是的，Aspose.Slides for Java 除了 XPS 之外，还提供多种输出格式，例如 PDF、TIFF 和 HTML。您可以通过更改 `SaveFormat` 调用时的参数 `save` 方法。请参阅文档以获取受支持格式的完整列表。

### 如何处理转换过程中的异常？

您可以实现异常处理，以便妥善处理转换过程中可能发生的任何错误。如代码所示， `try` 和 `finally` 即使发生异常，块也可以确保正确处置资源。
