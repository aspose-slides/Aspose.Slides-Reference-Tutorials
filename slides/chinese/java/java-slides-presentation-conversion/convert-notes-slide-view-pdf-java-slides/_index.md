...
description: 了解如何使用 Aspose.Slides for Java 将带注释的 PowerPoint 演示文稿转换为 PDF。按照我们提供的带源代码的分步指南进行操作。
weight: 16
...
## Java Slides 中将 Notes 幻灯片视图转换为 PDF 的简介
...
## 步骤 1：导入必要的类
首先，您需要从 Aspose.Slides 库导入必要的类。以下是执行此操作的代码：
...
您应该已经准备好 PowerPoint 演示文稿文件。替换`"Your Document Directory"`以及演示文稿文件所在目录的路径。以下是加载演示文稿的代码：
...
## 步骤 3：配置 PDF 选项
现在，让我们配置 PDF 导出选项。具体来说，我们将注释位置设置为“BottomFull”，以便在 PDF 中包含幻灯片下方的注释。以下是代码：
...
最后，让我们将演示文稿（包括笔记）保存为 PDF 文件。您可以指定输出文件名（例如，`"Pdf_Notes_out.pdf"`）并选择格式（`SaveFormat.Pdf`）。下面是执行此操作的代码：
...
## 在 Java 幻灯片中将 Notes 幻灯片视图转换为 PDF 的完整源代码
...
在本教程中，我们探索了如何使用 Aspose.Slides for Java 库将带有注释幻灯片视图的 PowerPoint 演示文稿转换为 PDF。我们遵循带有源代码的分步指南来实现此转换。以下是关键要点：
