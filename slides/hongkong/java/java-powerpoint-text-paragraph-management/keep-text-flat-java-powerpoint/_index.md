...
- 下載並安裝了 Aspose.Slides for Java 函式庫。您可以從以下位置獲取它：[這裡](https://releases.aspose.com/slides/java/).
...
首先載入 PowerPoint 簡報文件 (`pptxFileName`）並定義輸出路徑（`resultPath`) 對於處理後的幻燈片縮圖：
```java
...
```
## 第 2 步：存取和操作文字形狀
存取已載入簡報第一張投影片中的文字形狀 (`pres` ）。調整`KeepTextFlat`每個形狀對應的屬性：
```java
try {
